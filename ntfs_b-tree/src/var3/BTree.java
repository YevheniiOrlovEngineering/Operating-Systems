...
public class BTree<T extends Comparable<T>> implements ITree<T> {
     ...
     * 
     * @param node
     *            to remove the greatest value from.
    ...
    /**
     * Get the node with the search path.
     *
    ...
    public String search(T value) {
        ...
        while (node != null) {
            ...
            for (int i = 0; i < numberOfKeys; i++) {
                T currentValue = node.getKey(i);
                path.append(currentValue).append(" -> ");
                if (currentValue.compareTo(value) == 0) {
     ...
     * @return Node<T> child with the greatest value.
     */
    ...
    private boolean validateNode(Node<T> node) {
        ...
        if (node.parent == null) {
            ...
            if (keySize < minKeySize) {
                ...
                // If there are children, then there should be one more child
                // keys
                return false;
    ...
    public static class JavaCompatibleBTree<T extends Comparable<T>> extends java.util.AbstractCollection<T> {
        ...
        private static class BTreeIterator<C extends Comparable<C>> implements java.util.Iterator<C> {
            ...
            public boolean hasNext() {
                return (lastNode != null && index < lastNode.keysSize) || (toVisit.size() > 0);
            }
